...
class TheExecutiveSupabaseUser extends BaseAuthUser {
  TheExecutiveSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> theExecutiveSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TheExecutiveSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
